...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let key in ram_used) {
    var x = create_li_pair(
      key,
      (Math.round((ram_used[key] / 1024 / 1024) * 100) / 100).toFixed(2) +
        " MB",
      "performance",
      true
    );
  ...
  setInterval(function () {
    uptime.innerHTML =
      "Uptime: " + (Math.round(process.uptime() * 100) / 100).toFixed(2) + "s";
    for (let i = 0; i < ram_lists.length; i++) {
      var ram_used = process.memoryUsage();
      ram_lists[i].innerHTML =
        ram_lists[i].innerHTML.split(":")[0] +
        ": " +
        (
          Math.round(
            (ram_used[ram_lists[i].innerHTML.split(":")[0]] / 1024 / 1024) * 100
          ) / 100
        ).toFixed(2) +
        " MB";
    }
