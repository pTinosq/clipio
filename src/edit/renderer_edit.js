...
function add_clickable_module(module_name, module_function) {
  ...
  module_title_h1.setAttribute(
    "onclick",
    "clipboard.writeText(" + module_function + "); close_win();"
  );
...
function add_detected_colour() {
  ...
  colour_display.style = `background: ${getHexColour(
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  document.getElementById("to_replace").addEventListener("keyup", ({ key }) => {
    if (key === "Enter") {
      replace_module();
    }
  });
